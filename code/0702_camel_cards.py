...
def card_values() -> dict[str, int]:
    return {"A": 14, "K": 13, "Q": 12, "T": 10, "9": 9, "8": 8,
...
def count_jokers(card_str: str) -> int:
    return card_str.count("J")
...
def determine_hand(card_str: str) -> str:
    ...
    if cards[0] == cards[4]:
        ...
        hand = "High card"
    if "J" in card_str:
        n_jokers = count_jokers(card_str)
        hand = joker_adjustment(hand, n_jokers)
    return hand


def joker_adjustment(hand: str, n_jokers: int) -> str:
